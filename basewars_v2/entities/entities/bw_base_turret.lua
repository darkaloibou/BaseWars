...
if SERVER then
		...
		timer.Simple(0.15,function()
			...
			if tr.Entity == target then
				...
				timer.Simple(0.02,function()
					...
					self:ManipulateBoneAngles( mBonB, Angle(0,0,-3) )
					timer.Simple(0.04,function()
						if !IsValid( self ) then return end
						self:ManipulateBoneAngles( mBonB, Angle(0,0,-6) )
						timer.Simple(0.5,function()
							if !IsValid( self ) then return end
							self:ManipulateBoneAngles( mBonB, Angle(0,0,-9) )
							timer.Simple(0.6,function()
								if !IsValid( self ) then return end
								self:ManipulateBoneAngles( mBonB, Angle(0,0,-7) )
	...
	function ENT:Draw()
		...
		if !self:IsPowered( self.PowerMin ) then return end
		if self.beingPhysgunned and #self.beingPhysgunned > 0 then return end
