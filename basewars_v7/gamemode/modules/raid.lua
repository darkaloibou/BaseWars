...
function MODULE:CheckRaidable(ply, nocool, fac)
	...
	if not nocool then
		...
		if Cool < BaseWars.Config.Raid.CoolDownTime then return false, BaseWars.LANG.OnCoolDown .. " (" .. string.NiceTime(Left) .. " " .. BaseWars.LANG.Remaining .. ")" end
...
function MODULE:Start(ply, target)
	...
	hook.Run("RaidStart", ply, target, IsFaction)
	...
	for k,v in pairs(player.GetAll()) do if v:GetFaction() == P1Faction or v:GetFaction() == P2Faction then v:EmitSound("ambient/alarms/warningbell1.wav", 60) end end
...
function MODULE:Paint()
	...
	if TimeRemaining >= BaseWars.Config.Raid.Time - 5 then

		local sine = math.tan(RealTime() * 3) * 255
		surface.SetTextColor(255, sine, sine)
	...
	surface.SetTextPos(ScrW() / 2 - w / 2,h + 1)
	...
	surface.SetTextPos(ScrW() / 2 - w2 / 2,h + h2 + 2)
