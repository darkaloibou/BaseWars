...
local chill = Material("icon32/chill.png")
...
-- Karma
local karma = me.GetKarma and me:GetKarma() or 0

if karma >= 0 then
    draw.DrawText(karma, tag .. ".Large", respW(382), respH(1016), green, TEXT_ALIGN_LEFT)
else
    draw.DrawText(karma, tag .. ".Large", respW(382), respH(1016), red3, TEXT_ALIGN_LEFT)
end

draw.DrawText("Karma", tag .. ".Large", respW(322), respH(1016), white, TEXT_ALIGN_LEFT)

--CFC
local cfc = me.GetCFC and me:GetCFC() or 0
draw.DrawText(cfc, tag .. ".Large", respW(480), respH(995), turquoise, TEXT_ALIGN_LEFT)
draw.DrawText("Chill Factory Coin", tag .. ".Large", respW(322), respH(995), white, TEXT_ALIGN_LEFT)

-- time
draw.DrawText(os.date("%H:%M"), tag .. ".Large", respW(375), respH(975), yellow, TEXT_ALIGN_LEFT)
draw.DrawText("Heure", tag .. ".Large", respW(322.5), respH(975), white, TEXT_ALIGN_LEFT)
...
if CLIENT then
    ...
    local maxDistanceSqr = 300 * 300  -- Définir la distance maximale au carré
    ...
    hook.Add("PostDrawOpaqueRenderables", "AVHUD:DrawPlayerInfos", function()
        ...
        for _, v in ipairs(player.GetAll()) do
            ...
            local distSqr = ply:GetPos():DistToSqr(v:GetPos())  -- Calculer la distance au carré
            if distSqr <= maxDistanceSqr then  -- Vérifier si la distance au carré est inférieure ou égale à la distance maximale au carré
