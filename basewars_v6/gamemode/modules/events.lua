	...
	function MODULE:PayDay(ply)
		...
		if ply.AFKTime and ply:AFKTime() > 60 * 15 then
			...
			if count >= math.min(game.MaxPlayers() / 5, 10) then
				ply:Notify("Vous venez de manquer €" .. BaseWars.NumberFormat(Final) .. " parce que vous étiez AFK.", BASEWARS_NOTIFICATION_ERROR)
			else
				Final = math.floor(Final / 2)
				ply:Notify("Vous avez reçu un salaire réduit de €" .. BaseWars.NumberFormat(Final) .. " Pour la mise en service du serveur.", BASEWARS_NOTIFICATION_GENRL)
				ply:GiveMoney(Final)
			end
		else
