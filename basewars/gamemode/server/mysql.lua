...
function BaseWars.MySQL.GetLDB( ldbtype, callback )
	if BaseWars.MySQL.Enabled then
		if not BaseWars.MySQL.DbObj then
			...
			return BaseWars.MySQL.Connect(function()
				...
				BaseWars.MySQL.GetLDB( ldbtype, callback )
			end)
		end
		...
		local c = function( r )
			...
			if ldbtype == "time" then
				for _, v in ipairs( r[ 1 ].data ) do
					local pl = player.GetBySteamID64( v.sid64 )

					if !pl then
						newData[ v.sid64 ] = timeToStr( v.time )
					else
						newData[ v.sid64 ] = timeToStr( pl:GetPlayTime() )
					end
				end
			else
				newData = tonumber( r[ 1 ].data )
			end
	...
	for _, v in ipairs( sids ) do
		...
		if ldbtype == "time" then
			...
			if !pl then
				val[ v ] = timeToStr( dataType )
			else
				val[ v ] = timeToStr( pl:GetPlayTime() )
			end
		else
			val[ v ] = tonumber( dataType ) 
		end
	end
