...
ENT.Model = "models/mechanical_printer/mechanical_printer.mdl"
ENT.Skin = 0
...
ENT.UpgradeAugPercent 	= 0.5 // A chaque levelup, le prix augmente de 50%
...
if SERVER then
	...
	function ENT:UseFuncBypass(activator, caller, usetype, value)
		...
		if self:LocalToWorld(Vector(mins.x,mins.y , 0)):Distance(activator:GetEyeTrace().HitPos) < 15 then
		...
		elseif self:LocalToWorld(Vector(mins.x,maxs.y , 0)):Distance(activator:GetEyeTrace().HitPos) < 15 then
	...
	if CLIENT then
		...
		function ENT:DrawDisplay(pos, ang, scale)
			...
			draw.RoundedBox(0,-250,-175,500,350,Color(0,0,0))
			...
			if cap > 0 and cap ~= math.huge then
				...
				drawCircle(0, 0, 180, 100, (money / cap) * 360, 125, self.FontColor)
				drawCircle(0, 0, 180, 100, 360, 115, Color(0, 0, 0))
				draw.DrawText(self.PrintName, fontName, 0, -75, self.FontColor, TEXT_ALIGN_CENTER)
				draw.DrawText(string.format(BaseWars.LANG.LevelText, Lv):upper(), fontName .. ".Big", -225, -150, self.FontColor, TEXT_ALIGN_LEFT)
				draw.DrawText(string.format("Papier : ", paper):upper() .. paper, fontName .. ".Big", 225, -150, self.FontColor, TEXT_ALIGN_RIGHT)
				...
				draw.RoundedBox(10,-195,40,400,100,Color(0,0,0))

				draw.RoundedBox(10,-195,40,190,50,self.FontColor)
				draw.DrawText("Prendre l'argent", fontName .. ".Big", -100, 50, Color(0,0,0), TEXT_ALIGN_CENTER)

				draw.RoundedBox(10,20,40,160,50,self.FontColor)
				draw.DrawText("Améliorer", fontName .. ".Big", 100, 50, Color(0,0,0), TEXT_ALIGN_CENTER)
